...
def test_cost_func(cost, output, expected_output, expected_cost):
    assert np.allclose(cost.func(output, expected_output), expected_cost)
...
def test_cost_derivative(cost):
    ...
    assert np.allclose(numerical_derivative, cost.derivative(output, expected_output), rtol=1e-3)
